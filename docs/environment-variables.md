...
In this case, it is set to `iar` in order for Zephyr to use IAR's compiler toolchain to compile
applications.

`IAR_TOOLCHAIN_PATH` is used to set the path to the IAR compiler toolchain, which is needed for
the next steps when compiling an initial build using Zephyr tools. This is either EWARM's built in
compiler toolchain at `<path-to-your-EWARM-installation>\arm` or the standalone IAR ARM Build Tools
at `<path-to-your-iar-arm-build-tools-installation>\arm`.
...
[Next: Initial build and creating project](./creating-project.md)
